...
# 지갑 및 faucet 화면 가져오기
fFunc.wallet.extention()

# 기존에 생성한 계정(주소)을 seed 구문으로 가져오기
fFunc.wallet.importwallet()

# 네트워크 변수 적용하여 지갑 네트워크 세팅하기 (mdl 네트워크)
fFunc.wallet.settingnet(fFunc.network_name1, fFunc.rpc_url1, fFunc.chain_id1, fFunc.sign1)

# 네트워크 변수 적용하여 지갑 네트워크 세팅하기 (besu 네트워크)
fFunc.wallet.settingnet(fFunc.network_name, fFunc.rpc_url, fFunc.chain_id, fFunc.sign)

# 시작시간 측정하기
fFunc.faucet.starttime()
